...
# Assuming you have a PID class defined as follows
class PID:
...
pid_x = PID(Kp=0.1, Ki=0.001, Kd=0.05, setpoint=0.0)
pid_y = PID(Kp=0.1, Ki=0.001, Kd=0.05, setpoint=0.0)
pid_yaw = PID(Kp=0.15, Ki=0.01, Kd=0.005, setpoint=0)


class ros_tracker(Node):
    def __init__(self):
        super().__init__('ros_tracker')
        self.publisher_ = self.create_publisher(Float32MultiArray, 'tracked_pose', 10)
        self.tracked_object_pose = []
        print("ROS Tracker Node Initialized")

    def publish_pose(self):
        msg = Float32MultiArray()
        # Example list of floats
        msg.data = self.tracked_object_pose
        self.publisher_.publish(msg)
        self.get_logger().info('Publishing: {}'.format(msg.data))


ros_tracker_node = None
...
def compute_drone_action_while_tracking(mean_point, cfg, vehicle):
    ...
    if mean_point is None:
        ...
        try:
            ros_tracker_node.tracked_object_pose = [0, 0, 0, 0]
            ros_tracker_node.publish_pose()
        except:
            print("Failed to publish the coordinates to ROS")
        return
    ...
    normalized_center[x] = mean_point[x] / cfg['desired_width']  # obtain an x center between 0.0 and 1.0
    normalized_center[y] = mean_point[y] / cfg['desired_height']  # obtain a y center between 0.0 and 1.0
    normalized_center[z] = heading

    direction_vector = normalized_center - np.array([0.5, 0.5, 0])
    ...
    if cfg['use_filter']:
        postion_vector_queue.put(direction_vector)
    ...
    if postion_vector_queue.full():
        ...
        # Print the last coordinates
        print("Queue Full Filtered direction vector: ", p_filt[-1])

        # Compute PID outputs
        x_pid_output = pid_x.update(p_filt[-1][0])
        y_pid_output = pid_y.update(p_filt[-1][1])
        yaw_pid_output = pid_yaw.update(p_filt[-1][2])

        # Check if within threshold
        if abs(p_filt[-1][0]) < threshold and abs(p_filt[-1][1]) < threshold and abs(p_filt[-1][2]) < threshold:
            print("Target reached, stopping drone")
            x_pid_output = 0
            y_pid_output = 0
            yaw_pid_output = 0
        ...
        try:
            ros_tracker_node.tracked_object_pose = [x_pid_output, y_pid_output, 0.0, yaw_pid_output]
            ros_tracker_node.publish_pose()
        except:
            print("Failed to publish the coordinates to ROS")
    else:
        print("Queue not full direction vector: ", direction_vector)

        # Compute PID outputs
        x_pid_output = pid_x.update(direction_vector[0])
        y_pid_output = pid_y.update(direction_vector[1])
        yaw_pid_output = pid_yaw.update(direction_vector[2])

        # Check if within threshold
        if abs(direction_vector[0]) < threshold and abs(direction_vector[1]) < threshold and abs(direction_vector[2]) < threshold:
            print("Target reached, stopping drone")
            x_pid_output = 0
            y_pid_output = 0
            yaw_pid_output = 0
...
def track_object_with_aot(tracker, pred_mask, frame,  video, cfg, vehicle, track_single_object = True):
    ...
    with torch.cuda.amp.autocast():
        while 1: # 
            ...
            # Print the last coordinates
            print("Original Mean point obtained by tracker: ", mean_point)

            # # Publish the coordinates
            # try:
            #     ros_tracker_node.tracked_object_pose = mean_point
            #     ros_tracker_node.publish_pose()
            # except:
            #     print("Failed to publish the coordinates to ROS")
            # # ros_tracker_node.tracked_object_pose = mean_point
...
if __name__ == '__main__':
    ...
    # Create two  seperate threads for the drone and the main program
    thread_tracker = threading.Thread(target=start_mission, args=(device, tracker_cfg, cfg, tracker, detector, segmentor, queries, video, vehicle))
